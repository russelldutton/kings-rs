...
export const BaseLayout = ({ children }: PropsWithChildren) => {
  ...
  return (
    <div
      className={`${
        isDarkMode ? "dark" : ""
      } h-dvh w-dvh bg-background text-foreground`}
    >
      <div className="h-full w-full flex items-center bg-slate-500">
        <div className="container bg-muted h-2/3 w-2/3 px-12 py-8 border rounded-xl border-solid border-slate-500">
          <div className="flex gap-3">
            <Button onClick={() => setDarkMode((curr) => !curr)}>
              Toggle Dark
            </Button>
